...
function EditBanditModal({ isOpen, onClose, onUpdateBandit, bandit }) {
  ...
  // Charger les infractions disponibles et initialiser le formulaire
  useEffect(() => {
    if (isOpen && bandit) {
      loadInfractions();
      // Initialiser le formulaire avec les données du bandit
      let dateNaissance = '';
      if (bandit.dateNaissance) {
        // Si c'est déjà une date formatée (YYYY-MM-DD), l'utiliser directement
        if (typeof bandit.dateNaissance === 'string' && bandit.dateNaissance.includes('-')) {
          dateNaissance = bandit.dateNaissance.split('T')[0];
        } else {
          // Sinon, formater la date
          const date = new Date(bandit.dateNaissance);
      ...
      setFormData({
        ...
        sexe: bandit.sexe || 'M',
        etat: bandit.etat || 'CAPTURE',
        photo: null, // On ne charge pas la photo existante dans le file input
        infractions: bandit.infractions ? bandit.infractions.map(i => i.id) : [],
      });
      // Afficher la photo existante si elle existe
      if (bandit.photo) {
        setPhotoPreview(bandit.photo);
        setOriginalPhoto(bandit.photo);
      } else {
  ...
  const handleResponse = async (response) => {
    if (response.ok) {
      ...
      await Swal.fire({
        icon: 'success',
        title: 'Succès !',
        text: 'Bandit modifié avec succès',
      ...
      const errorMessage = errorData.message || 'Erreur lors de la modification du bandit';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Si une nouvelle photo est sélectionnée, convertir en base64
      if (formData.photo) {
        const reader = new FileReader();
        ...
        // Si pas de nouvelle photo, garder l'ancienne si elle existe
        if (originalPhoto) {
          dataToSend.photo = originalPhoto;
        } else {
      ...
      console.error('Erreur lors de la modification du bandit:', error);
      await Swal.fire({
        icon: 'error',
        title: 'Erreur',
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        <div className="relative bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-200">
            <h2 className="text-2xl font-bold text-gray-900">Modifier un bandit</h2>
            <button
              onClick={handleClose}
              ...
              <div>
                ...
                <div className="space-y-3">
                  {photoPreview && (
                    <div className="relative">
                      ...
                      <button
                        ...
                        title="Supprimer la nouvelle photo"
                      >
                  ...
                  <p className="text-xs text-gray-500">
                    Formats acceptés : JPG, PNG, GIF. Taille maximale : 5MB. Laissez vide pour conserver la photo actuelle.
                  </p>
                </div>
              </div>
...
export default EditBanditModal;
